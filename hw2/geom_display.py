...
def toHTML(divId,zoom,type,baseMapType,colorSet=0,redraw=False):
    # HTML
    html = ""
    if not redraw:
        html ="<div id='" + divId 
        html += """' style="width: 1000px; height: 600px"></div>"""
    html += """
        ...
        """
    if type=='':
        html += "(new GeoMap('" + divId  + "' ," + str(zoom) +","+str(baseMapType)+", '"+ str(colorSet)+ "')).display();</script>";
    elif type=='heatMap':
        html += "(new HeatMap('" + divId  + "' ," + str(zoom) +","+str(baseMapType)+", '"+ str(colorSet)+"')).display();</script>";
    elif type=='choroplethMap':
        html += "(new ChoroplethMap('" + divId  + "' ," + str(zoom) +","+str(baseMapType)+", '"+ str(colorSet)+"')).display();</script>";
    html = html.format('')
...
def display(results,divId, zoom,baseMapType=0,colorSet = 0):
    writeDataToFile(results,divId,type="");
    toHTML(divId,zoom,"",baseMapType,colorSet);
def heatMap(results,divId, zoom,baseMapType=0,colorSet = 0):
    writeDataToFile(results,divId,type="heatMap");
    toHTML(divId,zoom,"heatMap",baseMapType,colorSet);
def choroplethMap(results,divId, zoom,baseMapType=0,colorSet = 0):
    writeDataToFile(results,divId,type="choroplethMap");
    toHTML(divId,zoom,"choroplethMap",baseMapType,colorSet);
